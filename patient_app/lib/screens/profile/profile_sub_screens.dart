...
class _PaymentMethodsScreenState extends State<PaymentMethodsScreen> {
  ...
  Future<void> _load() async {
    try {
      final raw = await TripApiService.instance.getPaymentMethods();
      if (!mounted) return;
      setState(() {
        _methods = raw.cast<Map<String, dynamic>>();
        _loading = false;
      });
    } catch (_) {
  ...
  Future<void> _addMethod(String methodType, String identifier) async {
    Navigator.pop(context);
    try {
      await TripApiService.instance.addPaymentMethod(
  ...
  Future<void> _setDefault(String id) async {
    try {
      await TripApiService.instance.setDefaultPaymentMethod(id);
      await _load();
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  Future<void> _remove(String id) async {
    try {
      await TripApiService.instance.deletePaymentMethod(id);
      await _load();
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  void _showAddPaymentSheet(BuildContext context) {
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
      builder: (_) => _AddPaymentMethodSheet(onAdd: _addMethod),
    );
  }
      ...
      children: [
        if (_loading)
          ...
          _emptyState(
            ...
            subtitle: 'Link a payment method to easily pay for your rides.',
            buttonText: 'Add Payment Method',
            onButtonTap: () => _showAddPaymentSheet(context),
          )
        else ...[
          _settingsGroup(
            children: [
              for (int i = 0; i < _methods.length; i++) ...[
                ...
                _settingsTile(
                  icon: _methods[i]['method_type'] == 'CARD'
                      ? Icons.credit_card_outlined
                      : Icons.account_balance_wallet_rounded,
                  title: (_methods[i]['label'] ?? '').toString(),
                  subtitle: _methods[i]['is_default'] == true ? 'Default' : null,
                  onTap: () {},
                  trailing: PopupMenuButton<String>(
                    icon: const Icon(Icons.more_vert_rounded,
                        size: 20, color: cMutedLight),
                    onSelected: (action) {
                      final id = _methods[i]['id'].toString();
                      if (action == 'default') _setDefault(id);
                      if (action == 'remove') _remove(id);
                    },
                    itemBuilder: (context) => [
                      if (_methods[i]['is_default'] != true)
                        const PopupMenuItem(
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              icon: const Icon(Icons.add_rounded, color: cTeal),
              label: const Text('Add Payment Method',
                  style: TextStyle(
                      fontSize: 15, fontWeight: FontWeight.w700, color: cTeal)),
...
class _AddPaymentMethodSheet extends StatefulWidget {
  final void Function(String methodType, String identifier) onAdd;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Center(child: Container(width: 36, height: 4, decoration: BoxDecoration(color: cBorder, borderRadius: BorderRadius.circular(2)))),
          const SizedBox(height: 20),
          const Text('Link Payment Method', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: cTealDeep)),
          const SizedBox(height: 20),
          if (_methodType == null) ...[
            _settingsTile(
                ...
                onTap: () => setState(() => _methodType = 'MOBILE_MONEY')),
            const SizedBox(height: 12),
            _settingsTile(
            ...
            Text(
              _methodType == 'MOBILE_MONEY' ? 'Phone Number' : 'Card Number',
              style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: cMutedLight),
            ),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                hintText: _methodType == 'MOBILE_MONEY' ? '07XXXXXXXX' : '4242 4242 4242 4242',
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () {
                  ...
                  widget.onAdd(_methodType!, identifier);
                },
                style: ElevatedButton.styleFrom(backgroundColor: cTeal, foregroundColor: Colors.white, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
                child: const Text('Link', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
              ),
            ),
          ],
        ],
